<x-filament::page>
    <style>
        ...
        @media (min-width: 1024px) {
            ...
            .po-grid-5 { grid-template-columns: repeat(5, minmax(0, 1fr)); }
        }
    ...
    <div class="space-y-6">
        <x-filament::section class="po-section-gap">
            ...
            <div class="po-grid po-grid-3 po-section-pad">
                ...
                <div class="po-card">
                    ...
                    <div class="po-value">{{ data_get($data, 'display_id', '-') }}</div>
                </div>
                <div class="po-card">
                ...
                <div class="po-card">
                    ...
                    <div class="po-value">{{ data_get($data, 'total_amount', '-') }} {{ data_get($data, 'currency', '') }}</div>
                </div>
                <div class="po-card">
        ...
        <div class="po-grid po-grid-2">
            ...
            <x-filament::section class="po-section-gap">
                ...
                <div class="po-grid po-grid-2 po-section-pad">
                    <div class="po-kv">
                        <div class="po-label">Name</div>
                        <div class="po-value po-break">{{ data_get($data, 'seller.name', '-') }}</div>
                    </div>
                    <div class="po-kv">
                        <div class="po-label">Code</div>
                        <div class="po-value po-break">{{ data_get($data, 'seller.code', '-') }}</div>
                    </div>
                    <div class="po-kv">
                        <div class="po-label">Email</div>
        ...
        <x-filament::section class="po-section-gap">
            <x-slot name="heading">Order Details</x-slot>
            <div class="po-grid po-grid-5 po-section-pad">
                <div class="po-kv">
                    <div class="po-label">Quantity</div>
                    <div class="po-value">{{ data_get($data, 'order_details.quantity', '-') }}</div>
                </div>
                <div class="po-kv">
                    <div class="po-label">GRN Pass</div>
                    <div class="po-value">{{ data_get($data, 'order_details.grn_pass_quantity', '-') }}</div>
                </div>
                <div class="po-kv">
                    <div class="po-label">GRN Fail</div>
                    <div class="po-value">{{ data_get($data, 'order_details.grn_fail_quantity', '-') }}</div>
                </div>
                <div class="po-kv">
                    <div class="po-label">Put Away Pass</div>
                    <div class="po-value">{{ data_get($data, 'order_details.put_away_pass_quantity', '-') }}</div>
                </div>
                <div class="po-kv">
                    <div class="po-label">Put Away Fail</div>
                    <div class="po-value">{{ data_get($data, 'order_details.put_away_fail_quantity', '-') }}</div>
                </div>
            </div>
        </x-filament::section>
                ...
                <table class="po-table">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="po-num">Received</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($items as $item)
                            <tr>
                                <td>{{ data_get($item, 'sku.seller_sku_code', '-') }}</td>
                                <td>{{ data_get($item, 'sku.name', '-') }}</td>
                                <td class="po-num">{{ data_get($item, 'quantity', '-') }}</td>
                                <td class="po-num">{{ data_get($item, 'unit_price', '-') }}</td>
                                <td class="po-num">{{ data_get($item, 'received_quantity', '-') }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td class="po-empty" colspan="5">No items</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
